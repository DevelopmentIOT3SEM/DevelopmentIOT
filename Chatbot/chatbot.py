...
def chatbot(message: str) -> str:
    ...
    # ✅ Checar taxa de refugo ANTES de "refugo" genérico
    if "taxa" in message and "refugo" in message:
        return get_taxa_refugo()

    # ✅ Melhor detecção de sensores
    ...
    # Tempo ou total de produção
    elif "produção" in message or "produzido" in message:
        return get_total_operacoes()

    # Refugos
    elif "refugo" in message or "refugado" in message or "defeituoso" in message or "erro" in message:
        return get_total_refugos()
    
    # Horário/data de produção
    elif "data" in message or "horário" in message or "horario" in message or "quando" in message:
